...
void Renderer::requestRenderFrame(float *pointArray,int length) {
    pthread_mutex_lock(&mMutex);
    mPointsAdrr = pointArray;
    mArrayLength = length;
...
void Renderer::nativeSurfaceChanged(EGLint width, EGLint height) {
   if (width>height){
        projection = glm::ortho(-1.0f, 1.0f, 1.0f, -1.0f, -1.0f,
                                1.0f);//left,right,bottom,top near far
    }else{
        projection = glm::ortho(-1.0f, 1.0f, 1.0f, -1.0f, -1.0f,
                                1.0f);//left,right,bottom,top near far
    }
    //projection = glm::ortho(-1.0f, 1.0f, (float) width / height, -(float) width / height, 5.0f,
    //                        7.0f);//left,right,bottom,top near far
//    projection = glm::perspective(glm::radians(50.0f), (float)width/height, 5.0f ,7.0f);
    view = glm::lookAt(glm::vec3(0.0f, 0.0f, 6.0f),//摄像机位置（x,y,z）
                       glm::vec3(0.0f, 0.0f, 0.0f),//摄像机目标点（x,y,z）
                       glm::vec3(0.0f, 1.0f, 0.0f));//摄像机UP向量（x,y,z）相机正对着目标图像
...
void Renderer::nativeDraw() {
    ...
    float *mvp = (float *) glm::value_ptr(projection);
    // TODO
    mShape.draw(mvp,mPointsAdrr,mArrayLength);
}
